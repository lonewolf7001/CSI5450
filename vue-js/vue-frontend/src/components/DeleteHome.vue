      ...
      <div class="row3">
        ...
        <div class="col2">
          <Button
            ...
            text="Delete Home"
            color="lightred"
            v-on:click="this.deleteHomeResponse()"
          ></Button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    selectedHome() {
      this.selectedHome_details = this.homes.find(
    ...
    getHomeownerbyIDResponse() {
      OwnerService.getHomeownerbyID(
        this.selectedHome_details.currentOwner
      ).then((response) => {
        this.home_owner_details = response.data;
      });
    },
    deleteHomeResponse() {
      HomeService.deleteHomebyId(this.selected_homeId)
        .then((response) => {
